...
//  Today
//
//  Created by Likhitha Mandapati on 7/5/24.
//
...
class ReminderListViewController: UICollectionViewController {
    ...
    func pushDetailViewForReminder(withId id: Reminder.ID) {
        ...
        let viewController = ReminderViewController(reminder: reminder) { [weak self] updatedReminder in
            self?.updateReminder(updatedReminder)
            self?.updateSnapshot(reloading: [updatedReminder.id])
        }
